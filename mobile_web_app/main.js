...
function renderBillsTable() {
  ...
  bills.forEach(bill => {
    ...
    // show filter
    let include = true;
    let nextDue = computeNextDue(bill.startDate, bill.frequency);
    if (showFilter === 'due' && payStart) {
      include = isDueThisFortnight(bill, payStart);
    }
    if (!include) return;
...
// Re-render bills using the current filter/search; ensures edits show immediately.
function refreshBillsView() {
  // Re-render the bills table using the current filter/search criteria.
  // This wrapper avoids infinite recursion and ensures that
  // errors during rendering do not break the application. If an
  // error occurs, it is logged and we attempt to render once more.
  try {
    renderBillsTable();
  } catch (e) {
    console.warn('refreshBillsView error', e);
    try {
      renderBillsTable();
    } catch (_inner) {
      // If rendering fails again, swallow the error to prevent infinite loops.
    }
  }
}
...
async function addOrUpdateDebt() {
  ...
  const nameField = document.getElementById('debt-name');
  const owedField = document.getElementById('debt-owed');
  const minField  = document.getElementById('debt-minpay');
  const intField  = document.getElementById('debt-interest');
  const priField  = document.getElementById('debt-priority');

  if (!nameField || !owedField || !minField || !intField || !priField) {
    alert('Debt form not found on the page.');
    return;
  }

  const id = idField ? idField.value : '';
  const name = (nameField.value || '').trim() || 'Debt';
  const owed = Number.isFinite(parseFloat(owedField.value)) ? parseFloat(owedField.value) : 0;
  const minPay = Number.isFinite(parseFloat(minField.value)) ? parseFloat(minField.value) : 0;
  const interest = Number.isFinite(parseFloat(intField.value)) ? parseFloat(intField.value) : 0;
  const priority = (priField.value || 'Medium');

  if (owed < 0 || minPay < 0 || interest < 0) {
    alert('Please enter valid non‑negative numbers for owed, minimum payment, and interest.');
    return;
  }
  ...
  if (id) {
      ...
      if (typeof window.updateDebtCloud === 'function') {
        try {
          await window.updateDebtCloud(id, {
            name, amount: owed, minPayment: minPay, interest, priority
          });
        } catch (e) {
          console.error('Cloud update failed (debt). Keeping local change.', e);
        }
      }
    }
  } else {
    ...
    if (typeof window.insertDebtCloud === 'function') {
      try {
        ...
        if (remoteDebt && remoteDebt.id) newDebt.id = String(remoteDebt.id);
      } catch (e) {
        console.error('Cloud insert failed (debt). Saving locally anyway.', e);
      }
    }
    debts.push(newDebt);
  }
...
function calculateBudget() {
  ...
  bills.forEach(bill => {
    if (showFilter === 'due') {
      if (isDueThisFortnight(bill, payStart)) {
...
function renderDashboard() {
  ...
  if (payStart) {
    bills.forEach(b => {
      if (isDueThisFortnight(b, payStart)) dueBills.push(b);
    });
  }
  // Sort upcoming bills by next due date for display
  dueBills.sort((a, b) => {
    const da = computeNextDue(a.startDate, a.frequency);
    const db = computeNextDue(b.startDate, b.frequency);
