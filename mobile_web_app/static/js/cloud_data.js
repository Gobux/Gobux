...
(function () {
  ...
  async function fetchHistoryFromCloud() {
    ...
    // Map Supabase column names to our internal snapshot format. The
    // history table columns are: id, user_id, timestamp, pay_cycle,
    // income1, income2, splurge, bills, fire_pct, smile_pct,
    // remaining. We convert these to the keys used throughout
    // main.js: ts, pay_cycle, income1, income2, splurge, bills_due,
    // fire_pct, smile_pct, remaining. Any extra fields returned
    // by Supabase are passed through unchanged.
    return (data || []).map(row => {
      const mapped = { ...row };
      if (row.timestamp !== undefined) mapped.ts = row.timestamp;
      if (row.bills !== undefined) mapped.bills_due = row.bills;
      // Preserve original timestamp and bills keys for reference if needed
      return mapped;
    });
  }
  ...
  async function insertSnapshotCloud(snap) {
    const uid = await getUserId();
    // Map our internal snapshot keys to the Supabase column names.  The
    // history table columns are: id, user_id, timestamp, pay_cycle,
    // income1, income2, splurge, bills, fire_pct, smile_pct, remaining.
    const payload = {
      user_id: uid,
