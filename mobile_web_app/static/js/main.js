...
function clearBillForm() {
  ...
  const cf = document.getElementById('custom-frequency'); if (cf) cf.style.display='none';
...
function editBill(id) {
  ...
  const cf = document.getElementById('custom-frequency'); if (cf) cf.style.display = showCustom ? 'flex' : 'none';
  if (showCustom) {
    const cu = document.getElementById('custom-unit'); if (cu) cu.value = bill.customUnit || 'Week';
    const cv = document.getElementById('custom-value'); if (cv) cv.value = String(bill.customValue || '1');
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (freqSel) {
    freqSel.addEventListener('change', () => {
      const cf = document.getElementById('custom-frequency');
      if (cf) cf.style.display = (freqSel.value === 'Custom') ? 'flex' : 'none';
    });
  }
});
