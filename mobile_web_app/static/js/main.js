...
// Data containers loaded from localStorage. They persist between
// sessions so that your data is preserved when you close the page.
let bills = [];
let debts = [];
let goals = [];
let history = [];
...
function frequencyLabel(bill) {
  if (bill.frequency !== 'Custom') return bill.frequency;
...
function addOrUpdateBill() {
  ...
  if (id) {
    ...
    if (idx >= 0) {
      ...
      if (freq === 'Custom') { bills[idx].customUnit = cUnit; bills[idx].customValue = cValue; } else { delete bills[idx].customUnit; delete bills[idx].customValue; }
      bills[idx].startDate = start;
    }
  } else {
    const newBill = { id: String(Date.now()), name, amount, frequency: freq, startDate: start };
  if (freq === 'Custom') { newBill.customUnit = cUnit; newBill.customValue = cValue; }
  bills.push(newBill);
  }
  saveData();
  clearBillForm();
...
function editBill(id) {
  ...
  const showCustom = (bill.frequency === 'Custom');
  document.getElementById('custom-frequency').style.display = showCustom ? 'flex' : 'none';
  if (showCustom) {
...
function showDebtDeposit(id) {
  ...
  // Save handler
  document.getElementById('debt-deposit-save').onclick = () => {
...
function addOrUpdateGoal() {
  ...
  if (id) {
    ...
    goals.push({ id: String(Date.now()), name: name, targetAmount: target, savedAmount: 0, deadline: deadline, priority: priority });
  }
  saveData();
  clearGoalForm();
  renderGoalsTable();
...
function showGoalDeposit(id) {
  ...
  document.getElementById('goal-deposit-save').onclick = () => {
...
function importAllData(event) {
  ...
  reader.onload = function(e) {
    try {
      ...
      // Replace data containers if arrays exist
      bills = Array.isArray(data.bills) ? data.bills : bills;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (freqSel) {
    freqSel.addEventListener('change', () => {
      document.getElementById('custom-frequency').style.display = (freqSel.value === 'Custom') ? 'flex' : 'none';
    });
  }
});
